// 源文件头信息：
// <copyright file="MSOfficeConvertPdfHelper.cs">
...
namespace KenceryCommonMethod
{
    ...
    public static class MSOfficeConvertPdfHelper
    {
        ...
        public static void OfficeConvertPdfInfo(string sourcePath, string pdfPath)
        {
            ...
            {
                throw new Exception("该格式的MS Office 暂时不能转换为PDF!");
            }
        }
        ...
        private static void WorldConvertPdf(string worldPath, string pdfPath)
        {
            try
            {
                ...
                {
                    throw new Exception("不存在源文件，World不存在");
                }
                if (string.IsNullOrEmpty(pdfPath))
                {
                    throw new Exception("转换存放PDF的路径不能为空");
                }
            ...
            {
                throw new Exception("World转换PDF出现错误了，错误原因：" + exception.Message);
            }
        }
        ...
        private static void PptConvertPdf(string pptPath, string pdfPath)
        {
            try
            {
                ...
                {
                    throw new Exception("不存在源文件，PPT不存在");
                }
                if (string.IsNullOrEmpty(pdfPath))
                {
                    throw new Exception("转换存放PDF的路径不能为空");
            ...
            {
                throw new Exception("PPT转换PDF出现错误了，错误原因：" + exception.Message);
            }
        }
            ...
            float difinition)
        {
            try
            {
                ...
                {
                    throw new Exception("PDF的路径不存在");
                }
                //第二步：判断存放转换后图片的地址是否存在
                if (!Directory.Exists(imageOutpPutPath))
                {
                    throw new Exception("存放图片的路径不存在");
                }
