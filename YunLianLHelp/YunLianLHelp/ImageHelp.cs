...
{
    /// <summary>
    /// C# ImageHelper 操作图片的公共类
    /// <auther>
    ///     <name>秦效甫提供的共用类、韩迎龙做测试</name>
    ///     <date>2016-4-16</date>
    /// </auther>
    /// 图片各种各样的处理
    ///    1.缩略图
    ...
    public class ImageHelp
    {
        ...
        /// <param name="thumbnailPath">缩略图路径（物理路径）,(需要存放的图片路径)</param>
        /// <param name="thumbnailname">保存的图片名称</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式(HW:制定高宽缩放、W:指定宽高比例、H:指定高宽比例、Cut:定高宽裁减)</param>
        /// <param name="imagetype">生成的缩略图后缀</param>
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, string thumbnailname, int width, int height,
            string mode, ImageFormat imagetype)
        {
            ...
            //在指定位置并且按指定大小绘制原图片的指定部分(对哪张图片进行操作,画多么大,画那块区域,度量单位)
            g.DrawImage(originalImage, new Rectangle(x, y, towidth, toheight), new Rectangle(x, y, ow, oh), GraphicsUnit.Pixel);
            try
            {
                //构造路径，保存图片
                bitmap.Save(thumbnailPath + thumbnailname + "." + imagetype.ToString(), imagetype);
            }
            catch (Exception e)
            {
        ...
        public static ArrayList GetLocation(string location, Image img, int width, int height)
        {
            #region
